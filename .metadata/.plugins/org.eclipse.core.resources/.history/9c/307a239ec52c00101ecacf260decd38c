...
//		return null;
	...
	{
		boolean status = addExamsService.upComingExam(exam_id);
	
		if(status)
		{ 
			return ResponseEntity.ok("Appceted")
					;

		}
		else {
			 return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Data not Found the Database");
		}
