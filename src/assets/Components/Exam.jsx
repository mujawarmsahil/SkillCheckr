        ...
        <div>
            ...
            {exams.length > 0 ? (
                <ul className="flex flex-wrap gap-[30px] my-2">
                    {exams.map((exam) => (
                        <li key={exam.exam_id} className="w-[300px] h-[150px] flex gap-2 flex-col bg-orange-50 p-2 rounded-sm shadow-[0_0_5px_1px_black]">
                            <strong>{exam.exam_name}</strong> - {exam.subject_name} <br />
                            <em>{formatDate(exam.exam_date)}</em>
                            <button type="button" className="bg-orange-400 w-[100px] mx-auto hover:bg-orange-500 text-white p-1  rounded">Start exam</button>
                        </li>
                    ))}
                </ul>
            ) : (
